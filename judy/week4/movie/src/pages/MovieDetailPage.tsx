...
export default function MovieDetailPage () : React.ReactElement {
    ...
    const { data: movieDetails, isPending: isDetailsPending, isError: isDetailsError, error: detailsError } = useCustomFetch<MovieDetails>(
        `https://api.themoviedb.org/3/movie/${movieId}?language=ko-KR`
    );

    const { data: credits, isPending: isCreditsPending, isError: isCreditsError, error: creditsError } = useCustomFetch<Credits>(
    ...
    if (isError) {
        return <ErrorMessage message={error || '영화 정보를 불러오는 중 에러가 발생했습니다.'} />
    }
