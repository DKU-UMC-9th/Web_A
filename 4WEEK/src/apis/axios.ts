...
//전역 변수로 refresh 요청의 Promiss를 저장해서 중복 요청을 방지한다
...
axiosInstance.interceptors.request.use((config)=>{
    ...
    //accssToken이 존재하면 Authorization헤더에 Bearer 토큰 형식으로 추가한다
    if(accessToken){
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    // 401에러이면서, 아직 재시도하지 않은 요청의 경우 처리
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      // refresh 엔드포인트 자체가 401이면 -> 로그아웃 처리
      ...
      if (!refreshPromise) {
        refreshPromise = (async () => {
          ...
          // 새 accessToken을 반환하여 다른 요청들이 이것을 사용하게 함
